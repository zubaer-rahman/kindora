...
OrganizationProfileSchema.pre(
  "save",
  async function (this: IOrgnizationPofile) {
    if (this.opportunity_types.length === 0) {
      throw new Error("At least one opportunity type is required");
    }
...
const OrganizationProfile =
  mongoose.models.organization_profile ||
  mongoose.model<IOrgnizationPofile>(
