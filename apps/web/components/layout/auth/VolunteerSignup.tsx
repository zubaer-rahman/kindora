...
import { ProfileSetupPrompt } from "@/components/layout/auth/ProfileSetupPrompt";
...
export default function VolunteerSignup() {
  ...
  const { isLoading, isProfileSetupComplete, setupProfile, signup } = useVolunteerSignup();
  ...
  const handleSignup = async () => {
    ...
    const fieldsToValidate: Array<keyof VolunteerSignupForm> = [
      "name",
      "email",
      "password",
      "confirm_password",
  ...
  if (needsProfileSetup && !isAuthenticated && !isProfileSetupComplete) {
    return (
      <div className="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 pb-32">
        <ProfileSetupPrompt role="volunteer" isLoading={isLoading} onSetupProfile={setupProfile} />
      </div>
    );
  }
