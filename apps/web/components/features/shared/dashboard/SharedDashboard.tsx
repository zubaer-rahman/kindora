...
import TabContent from "@/components/features/organization/dashboard/TabContent";
import VolunteerCarousel from "@/components/features/organization/dashboard/VolunteerCarousel";
import MessageDialog from "@/components/features/organization/MessageDialog";
import ConfirmationDialog from "@/components/modals/ConfirmationDialog";
import { useSessionRefresh } from "@/components/features/organization/dashboard/hooks/useSessionRefresh";
import { Volunteer, ActiveContract } from "@/types/organization";

export interface SharedDashboardProps {
  role: "organisation" | "mentor";
  opportunities: Opportunity[];
  isLoadingOpportunities: boolean;
  isOpportunitiesError: boolean;
  recruitedApplicants: any[];
  availableVolunteers: any[];
  isLoadingVolunteers: boolean;
  archiveMutation?: { mutate: (id: string, opts: any) => void; isPending: boolean };
  unarchiveMutation?: { mutate: (id: string, opts: any) => void; isPending: boolean };
...
const SharedDashboard = ({
  ...
  for (const c of recruitedApplicants ?? []) {
    ...
    if (existing) {
      if (c.opportunity?.id && !existing.opportunities.some((o: any) => o.id === c.opportunity!.id)) {
  ...
  const handleConfirm = () => {
    ...
    if (pendingAction === "delete" && deleteMutation) {
      deleteMutation.mutate(id, { onSettled: closeDialog });
    } else if (pendingAction === "archive" && archiveMutation) {
      archiveMutation.mutate(id, { onSettled: closeDialog });
    } else if (pendingAction === "unarchive" && unarchiveMutation) {
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 md:py-12 bg-background min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8">
        <h2 className="text-lg md:text-xl font-bold tracking-tight">
          {getGreeting()}, {session?.user?.name || (role === "mentor" ? "Mentor" : "Org Name")}
        </h2>
        {role === "organisation" && <CreateOpportunityButton />}
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
        ...
        <button
          ...
          onClick={() => router.push(`/${role}/manage-opportunities`)}
        >
