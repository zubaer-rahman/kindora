...
export function MentorOpportunityCard({ opportunity }: MentorOpportunityCardProps) {
    ...
    const handleCardClick = () => {
        if (session) {
            router.push(`/volunteer/opportunities/${opportunity._id}/details`);
        } else {
            router.push(`/login?redirect=/volunteer/opportunities/${opportunity._id}/details`);
        }
    };
    ...
    return (
        <Card
            ...
            className="group hover:bg-muted/50 transition-colors rounded-none border-x-0 p-0 border-t-0 border-b border-border bg-card cursor-pointer flex flex-col w-full shadow-none"
        >
            <CardContent className="px-4 py-6 flex flex-col flex-1">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-4 text-[13px] text-muted-foreground">
                    <span className="font-medium text-foreground">{opportunity.commitment_type || "N/A"}</span>
                    ...
                    <div className="flex-1 flex gap-2 overflow-x-auto no-scrollbar px-1">
                        {opportunity.category?.slice(0, 3).map((cat: string, index: number) => (
                            <Badge
                                key={index}
                                variant="secondary"
                                className="bg-secondary/50 text-secondary-foreground hover:bg-secondary/70 border-none px-3 py-1 text-xs font-medium rounded-full whitespace-nowrap"
                            >
                                {cat}
                            </Badge>
                        ))}
                        {opportunity.category && opportunity.category.length > 3 && (
                            <Badge
                                variant="secondary"
                                className="bg-secondary/50 text-secondary-foreground border-none px-3 py-1 text-xs font-medium rounded-full"
                            >
                ...
                <div className="text-[13px] text-muted-foreground">Click to manage volunteers</div>
            </CardContent>
        </Card>
    );
}
