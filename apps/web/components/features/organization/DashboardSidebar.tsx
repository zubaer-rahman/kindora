...
import { LayoutDashboard, Briefcase, MessageSquare, Settings, Building2 } from "lucide-react";
...
export default function OrganisationDashboardSidebar() {
  ...
  const { data: organisation, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosAuth.get("/api/v1/users/me/organisation-profile");
    ...
    <div className="w-full lg:w-[350px] flex flex-col gap-6">
      ...
      <div className="bg-accent rounded-[24px] p-6 border border-border">
        <div className="flex items-center gap-4 mb-4">
          <UserAvatar user={{ name: organisation?.name || "Organisation", image: organisation?.image }} size={64} className="w-16 h-16 rounded-2xl" />
          <div>
            <h3 className="text-lg font-semibold text-foreground">{organisation?.name || "Organisation"}</h3>
